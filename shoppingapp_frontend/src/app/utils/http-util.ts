...
export class HttpUtil {
    baseUrl = "http://localhost:8090/api/v1.0/shopping";
    constructor(private http: HttpClient) {
    }

    saveDetailsToDatabase(registerRequestJson: any): Observable<UserDetails> {
        return this.http.post('http://localhost:8090/api/v1.0/shopping/register', registerRequestJson);
    }

    getUserDetailsFromDatabase(loginRequestJson: any): Observable<UserDetails> {
        return this.http.post('http://localhost:8090/api/v1.0/shopping/login', loginRequestJson);
    }
